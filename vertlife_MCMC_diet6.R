...
if(diet.coding == "conserve") {
  ...
  #unique(sif[sif$conservative_coding != "NO", diets])
  #Arthropods Blood Terrestrial.vertebrates Fish Leaves.and.flower.pieces Pollen.and.nectar Fruit Seed
  #         2     0                       0    0                        0                 0     1    0
  #         2     0                       1    0                        0                 0     1    0
...
## add diets of the vampire ancestral nodes
sif = rbind(sif, 0)
sif[nrow(sif), c("vertlife_name", "Blood")] = c("Desmodontinae", 3)
sif = rbind(sif, 0)
sif[nrow(sif), c("vertlife_name", "Blood")] = c(getMRCA(tree, c("Desmodus_rotundus", "Diaemus_youngi")), 3)
...
fit <- MCMCglmm(
  ...
  cbind(Arthropods, Blood, Terrestrial.vertebrates, Fish, Pollen.and.nectar, Fruit) ~ trait - 1,
